...
import argparse, pprint, re, requests, sys, signal
...
parser.add_argument('-o', '--port', action='store_true', default=8000, help='port number for metrics')
...
class Collector():
    def collect(self):
        gauge_tx_good_pkt = GaugeMetricFamily('tplink_tx_good_pkt', 'tplink tx good packets',labels=['target','port'])
        gauge_tx_bad_pkt = GaugeMetricFamily('tplink_tx_bad_pkt', 'tplink tx bad packets',labels=['target','port'])
        gauge_rx_good_pkt = GaugeMetricFamily('tplink_rx_good_pkt', 'tplink rx good packets',labels=['target','port'])
        gauge_rx_bad_pkt = GaugeMetricFamily('tplink_rx_bad_pkt', 'tplink rx bad packets',labels=['target','port'])
        gauge_link_status = GaugeMetricFamily('tplink_link_status', 'tplink link status',labels=['target','port'])
        gauge_state = GaugeMetricFamily('tplink_state', 'tplink state',labels=['target','port'])
        ...
        for row in jlist:
            ...
            # apply
            gauge_tx_good_pkt.add_metric([args['target'],port],tx_good_pkt)
            gauge_tx_bad_pkt.add_metric([args['target'],port],tx_bad_pkt)
            gauge_rx_good_pkt.add_metric([args['target'],port],rx_good_pkt)
            gauge_rx_bad_pkt.add_metric([args['target'],port],rx_bad_pkt)
            gauge_link_status.add_metric([args['target'],port],link_status)
            gauge_state.add_metric([args['target'],port],state)
